...
who = "Holly"
age = 9
s4 = f"{who.upper()} turned {age:>2} this year"
print("f:", s4)         # HOLLY turned  9 this year

'''
Заполнитель {age:>2} использует спецификатор формата >2, чтобы указать, что значение года должно быть выровнено по правому краю (>) в
поле ширины 2 — ширина поля определяет количество позиций символов, которые будут использоваться при отображение значения. 
Для значений 1–9 описатель формата >2 отображает пробел, за которым следует значение, таким образом выравнивая годы в первом столбце
 по правому краю.'''

principal, rate, year = 12.45, 3.5, 2
amount = principal * (1 + rate) ** year
print(f'Для {year:>2} лет, сумма: {amount:>10.2f}')

'''Спецификатор формата 10.2f в заполнителе {amount}:>10,2f} форматирует сумму как число с плавающей запятой (f), выровненное 
по правому краю (>) в поле шириной 10 от десятичной точки и две цифры справа от десятичной точки (.2). 
'''
